...
public class CustomerService {
  ...
  public CustomerDTO findByEmail(String email) throws FinderException {
    Customer customer =
        customerRepository
            .findByEmail(email)
            .orElseThrow(() -> new FinderException("Customer with email {" + email + NOTFOUNDMSG));
    if (customer == null) {
  ...
  public CustomerDTO findByPhone(String phone) throws FinderException {
    Customer customer =
        customerRepository
            ...
            .orElseThrow(() -> new FinderException("Customer with phone {" + phone + NOTFOUNDMSG));
    if (customer == null) {
    ...
    try {
      ...
      if (customerExists(email, phoneNumber)) {
        throw new DuplicateKeyException(
            "Customer with email" + email + " and phone " + phoneNumber + " already exists");
      }
  ...
  public CustomerDTO updateCustomer(Long id, CustomerDTO customerUpdate)
    ...
    try {
      ...
      throw new IllegalArgumentException(CUSTMSG + id + "} update failed" + e.getMessage(), e);
    } catch (Exception e) {
