...
public class EmployeeService {
  ...
  public EmployeeDTO updateEmployee(Long id, EmployeeDTO employeeDTOUpdate)
    ...
    try {
      ...
      throw new IllegalArgumentException(EMPMSG + id + "} update failed : " + e.getMessage(), e);
    } catch (Exception e) {
  ...
  public EmployeeDTO patchEmployee(Long id, Map<String, Object> employeePatch)
    ...
    try {
      Optional<Employee> optionalEmployee = employeeRepository.findById(id);
      if (optionalEmployee.isPresent()) {
        employeePatch.forEach(
            (key, value) -> {
              if (key != null) {
                Field field = ReflectionUtils.findField(Employee.class, key);
                if (field != null) {
                  ReflectionUtils.makeAccessible(field);
                  if (value != null) ReflectionUtils.setField(field, optionalEmployee.get(), value);
                }
              }
            });
      ...
      throw new IllegalArgumentException(EMPMSG + id + "} patch failed : " + e.getMessage(), e);
    } catch (Exception e) {
