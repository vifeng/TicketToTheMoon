...
public class HallService {
  ...
  public HallService(
      HallRepository hallRepository, VenueRepository venueRepository, HallMapper hallMapper) {
      ...
      if (hallOptional.isPresent()) {
        ...
        if (hallDTO.venue() != null) {
          ...
          if (venue.isPresent()) {
            ...
            throw new FinderException("Venue with id {" + hallDTO.venue().id() + NOTFOUNDMSG);
          }
        }
  ...
  public HallDTO createHall(Long venueId, HallDTO hallDTO)
    ...
    if (!Objects.equals(venueId, hallDTO.venue().id()))
      throw new IllegalArgumentException(
          "IllegalArgumentException : Venue id {"
              + venueId
              + "} and HallDTO venue id {"
              + hallDTO.venue().id()
  ...
  public List<HallDTO> findHallsByVenueId(Long venueId) throws FinderException {
    ...
    int size = ((List<Hall>) halls).size();
    if (size == 0) {
      throw new FinderException("No Halls in the database for Venue with id {" + venueId + "}");
    }
    return hallMapper.toDTOs(halls);
  }
}
